...
if (!class_exists('G5Staff_ShortCodes')) {
    class G5Staff_ShortCodes
    {
        ...
        public function get_shortcodes() {
            return apply_filters('g5staff_shortcodes',array(
                'staff',
                'staff_slider'
            ));
        }
        ...
        public function vc_lean_map_config($vc_map_config,$key) {
            if (in_array($key,$this->get_shortcodes())) {
                ...
                $vc_map_config = G5STAFF()->locate_template("shortcodes/{$file_name}/config.php");
            }
        ...
        public function change_autoload_class_path($path,$shortcode,$file_name) {
            if (in_array($shortcode,$this->get_shortcodes())) {
                $path = G5STAFF()->locate_template("shortcodes/{$file_name}/{$file_name}.php");
            }
            return $path;
        }
        ...
        public function change_shortcode_template($template, $template_name) {
            if (in_array($template_name,$this->get_shortcodes())) {
                ...
                $template = G5STAFF()->locate_template("shortcodes/{$template_name}/template.php");
            }
            return $template;
        }

        public function get_auto_complete_fields() {
            return apply_filters('g5staff_auto_complete_fields',array(
                'g5element_staff_ids',
	            'g5element_staff_slider_ids',
        ...
        public function set_query_args($query_args,$atts) {
            if ($query_args['post_type'] === 'staff') {
                ...
                if ($atts['show'] !== 'staff' && !empty($atts['cat'])) {
                    $query_args['tax_query'][] = array(
