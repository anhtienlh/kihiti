...
$layout_matrix = G5STAFF()->listing()->get_layout_matrix($post_layout);
$image_size = isset($post_settings['image_size']) ? $post_settings['image_size'] : (isset($layout_matrix['image_size']) ? $layout_matrix['image_size'] : G5STAFF()->options()->get_option('post_image_size'));
...
$image_ratio = '';
if ($image_size === 'full') {
    $image_ratio_custom = isset($post_settings['image_ratio']) ? $post_settings['image_ratio'] : G5STAFF()->options()->get_option('post_image_ratio');
...
$wrapper_classes = array(
    'g5staff__listing-wrap',
    "g5staff__layout-{$post_layout}"
);
...
$inner_classes = array(
    'g5staff__listing-inner'
);

$post_classes = array(
    'g5core__gutter-item',
    'g5staff__post-default',
    $item_custom_class,
    ...
    <div <?php echo join(' ', $inner_attributes); ?> class="<?php echo esc_attr($inner_class); ?>">
        ...
        if ($layout_settings !== '') {
            ...
            while (G5CORE()->query()->have_posts()) {
                ...
                $template_class = isset($current_layout['template_class']) ? $current_layout['template_class'] : "g5staff__post-{$template}";
                $post_index = intval(G5CORE()->query()->get_query()->current_post) +1;
                $current_post_classes = array(
                    $template_class,
                    "g5staff__post-item-{$post_index}"
                );
                ...
                G5STAFF()->get_template("loop/listing/item/{$template}.php",array(
            ...
            G5STAFF()->get_template( "loop/listing/{$post_layout}.php",array(
...
<?php elseif (isset($post_settings['isMainQuery'])): ?>
    <?php G5STAFF()->get_template( 'loop/content-none.php' ); ?>
<?php endif; ?>
