...
$layout_matrix = G5PORTFOLIO()->listing()->get_layout_matrix($post_layout);
$image_size = isset($post_settings['image_size']) ? $post_settings['image_size'] : (isset($layout_matrix['image_size']) ? $layout_matrix['image_size'] : G5PORTFOLIO()->options()->get_option('post_image_size'));
...
if ($post_layout === 'masonry') {
    $image_width = isset($post_settings['image_width']) ?  $post_settings['image_width'] : G5PORTFOLIO()->options()->get_option('post_image_width');
...
$image_ratio = '';
if ($image_size === 'full') {
    $image_ratio_custom = isset($post_settings['image_ratio']) ? $post_settings['image_ratio'] : G5PORTFOLIO()->options()->get_option('post_image_ratio');
...
$wrapper_classes = array(
    'g5portfolio__listing-wrap',
    "g5portfolio__layout-{$post_layout}"
);
...
$inner_classes = array(
    'g5portfolio__listing-inner'
);

$post_classes = array(
    'g5core__gutter-item',
    'g5portfolio__post-default',
    $item_custom_class,
);
if ($justified !== '') {
    ...
    <div <?php echo join(' ', $inner_attributes); ?> class="<?php echo esc_attr($inner_class); ?>">
        ...
        if ($layout_settings !== '') {
            ...
            while (G5CORE()->query()->have_posts()) {
                ...
                $template_class = isset($current_layout['template_class']) ? $current_layout['template_class'] : "g5portfolio__post-{$template}";
                $post_index = intval(G5CORE()->query()->get_query()->current_post) +1;
                $current_post_classes = array(
                    $template_class,
                    "g5portfolio__post-item-{$post_index}"
                );
                ...
                G5PORTFOLIO()->get_template("loop/listing/item/{$template}.php",array(
            ...
            G5PORTFOLIO()->get_template( "loop/listing/{$post_layout}.php",array(
...
<?php elseif (isset($post_settings['isMainQuery'])): ?>
    <?php G5PORTFOLIO()->get_template( 'loop/content-none.php' ); ?>
<?php endif; ?>
